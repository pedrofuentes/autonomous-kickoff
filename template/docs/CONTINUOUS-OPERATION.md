...
- **Startup banner.** When running attended, say so in one line — "running attended single-operator (own identity); board decision channel bounded-trusted (self-signature + cofounder-login + solo-repo), Tier-2 disabled; provision a distinct identity to go unattended" — so the reduced posture is never silent.
