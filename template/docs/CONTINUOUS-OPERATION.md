...
**3. Agent consumes it (each watchdog tick).** For every open gate card: a `needs:decision` card → check for a cofounder decision newer than the request — a `decision:approved` / `decision:changes` **label** (confirm the `labeled` event's `actor` is the cofounder via `gh api /repos/{o}/{r}/issues/{n}/events`) or a first-line `Decision:` comment that is **not** self-signed (`gh issue view <n> --comments`); a `blocked` card → re-check the actual state. When resolved: record it (in the issue, and in `DECISIONS.md` if it's an architectural choice), remove the gate label, **move the gate card to Done and close it**, move the affected work card back to **In Progress / Todo** (restore its `ready` or `claimed:*` label), and proceed. Not resolved yet → leave it and keep working elsewhere.
...
**Separate identity (recommended for durable runs).** The cleanest fix is to give the agent its own GitHub identity so author attribution is trustworthy: run it as the **Copilot coding agent** (acts as `Copilot`), a **private GitHub App** (`<app-slug>[bot]`), or — in a Tier-2 Actions workflow — the default `GITHUB_TOKEN` (`github-actions[bot]`). Each makes the agent `user.type == "Bot"`, distinct from the cofounder, and lets GitHub enforce review gates (a PR author cannot approve their own PR). A personal access token (classic or fine-grained) does **not** separate identity — it still acts as the cofounder, which is why the self-signature marker above is the required baseline.
...
- **Milestone done:** the current milestone's DoD is met (incl. no open high/critical security alert or detected secret) and its board is empty → **don't stop**; open the next-milestone `DECISION:` gate and resume on approval.
