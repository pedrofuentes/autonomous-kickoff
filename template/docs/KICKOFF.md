...
2. **Propose — and shape — the next milestone** via the **Decision protocol** (`CONTINUOUS-OPERATION.md` Tier 3): draft the next milestone's scope (from `ROADMAP.md` + what you learned this milestone) into a `DECISION:` issue (Status **Pending Decision**, `needs:decision`) with features + acceptance + your recommendation; @-mention the cofounder, and refine it with their feedback (live in the CLI, or via `Decision: changes — …` on the issue), recording the agreed scope in `ROADMAP.md`. **Do not start building it** — new milestone scope is a gated decision.
3. **Idle pending the decision** with the watchdog armed (don't hard-stop while the roadmap still has milestones).
4. **On the answer:** `Decision: approved` or `Decision: option <X>` → set the new milestone's DoD, **re-seed the board** from `ROADMAP.md`, run only the phases the new scope needs (re-run Discovery/UX/Architecture if it introduces them; otherwise go straight to Build), and resume. `Decision: changes — …` → revise the proposed scope, update `ROADMAP.md`, re-post the proposal, and **stay Pending Decision** until approved (don't build yet). `Decision: hold` → stay paused. If the cofounder declares the **project complete** or the roadmap is exhausted, verify the final state — **then** stop (the one clean full stop).
