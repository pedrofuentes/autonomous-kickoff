...
| **Test / QA** | `general-purpose` / `task` sub-agents | Test data, e2e, accessibility + performance/security audits; runs the **write-test → run → debug → re-run** loop, routing failures back to the engineer | Tests are first-class; coverage ratchets up; complements per-increment unit TDD, doesn't replace it |
